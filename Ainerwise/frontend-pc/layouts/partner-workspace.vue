    ...
    <header class="sticky top-0 z-40 border-b border-blue-400/15 bg-slate-950/90 backdrop-blur">
      <div class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        <div class="flex min-w-0 items-center gap-4">
          <NuxtLink to="/partner" class="shrink-0">
          ...
          <nav class="hidden items-center gap-1 lg:flex">
            <NuxtLink v-for="item in nav" :key="item.to" :to="item.to" class="rounded-lg px-3 py-2 text-xs font-semibold text-slate-400 hover:bg-white/5 hover:text-white" active-class="!bg-blue-500/15 !text-blue-200">
              {{ item.label }}
            </NuxtLink>
          </nav>
        </div>
      ...
      <nav class="flex gap-1 overflow-x-auto border-t border-white/5 px-4 py-2 lg:hidden">
        <NuxtLink v-for="item in nav" :key="item.to" :to="item.to" class="shrink-0 rounded-lg px-3 py-2 text-xs font-semibold text-slate-400" active-class="!bg-blue-500/15 !text-blue-200">
          {{ item.label }}
        </NuxtLink>
      </nav>
    </header>
