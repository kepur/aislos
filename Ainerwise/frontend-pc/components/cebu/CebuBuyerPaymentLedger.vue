<template>
  <section class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-indigo-300">Payments</p>
        <h1 class="mt-1 text-2xl font-bold text-white">支付与结算账本</h1>
        <p class="mt-2 max-w-2xl text-sm text-slate-400">AISLOS 仅记录 PSP 支付/结算状态，不托管资金、不虚构余额；线下到账由管理员确认后入账。</p>
      </div>
    ...
    <div class="pc-card">
      <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h3 class="text-lg font-medium text-white">账目明细</h3>
        ...
        <table class="w-full min-w-[760px] text-left text-sm">
          <thead class="text-slate-400">
            <tr class="border-b border-white/10">
              ...
              <th class="py-2 font-medium">参考号</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-if="!loading && !filtered.length">
              <td colspan="6" class="py-8 text-center text-slate-500">暂无支付记录</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const summary = computed(() => {
  ...
  return [
    { label: '账目记录', value: items.value.length, hint: '全部支付/结算条目' },
    { label: '流入合计', value: money(inflow, currency), hint: '退款 / 入账' },
    { label: '流出合计', value: money(outflow, currency), hint: '付款 / 手续费' },
    { label: '已结算', value: settled, hint: `共 ${items.value.length} 条` },
  ]
})
...
async function load() {
  ...
  try {
    ...
    error.value = e?.data?.detail || e?.message || '加载账本失败'
  } finally {
    loading.value = false
  }
}
