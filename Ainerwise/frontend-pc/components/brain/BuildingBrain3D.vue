...
const sceneConfigs: Record<string, { label: string; chips: string[]; buildings: number; solar: boolean; ev: boolean; robots: boolean; doors: boolean; garden: boolean; campus: boolean; industrial?: boolean }> = {
  ...
  factory: { label: 'Factory / Industrial Plant', chips: ['Factory AI', 'PLC/SCADA', 'Machines', 'Energy'], buildings: 7, solar: true, ev: true, robots: true, doors: true, garden: false, campus: false, industrial: true },
...
function addBuildings(group: THREE.Group, color: number, secondary: number) {
  ...
  const scenarioTones: Record<string, { base: number; glass: number; accent: number; window: number }> = {
    ...
    factory: { base: 0x171717, glass: 0x27272a, accent: 0xf97316, window: 0xfacc15 },
  ...
  const layouts: Record<string, BuildingSpec[]> = {
    ...
    factory: [
      { x: -1.95, z: -0.96, w: 1.42, h: 0.82, d: 0.72, roof: 'antenna' },
      { x: -0.48, z: -0.98, w: 1.42, h: 0.82, d: 0.72, roof: 'flat' },
      { x: 0.98, z: -0.98, w: 1.28, h: 0.82, d: 0.72, roof: 'flat' },
      { x: 2.0, z: -0.88, w: 0.46, h: 1.72, d: 0.46, roof: 'antenna' },
      { x: -1.62, z: -1.82, w: 0.94, h: 0.48, d: 0.46, roof: 'battery' },
      { x: -0.4, z: -1.84, w: 0.94, h: 0.48, d: 0.46, roof: 'battery' },
      { x: 0.82, z: -1.84, w: 0.94, h: 0.48, d: 0.46, roof: 'battery' },
    ],
    hotel: [
  ...
  for (const [i, spec] of specs.entries()) {
    const material = props.scenarioKey === 'office' || props.scenarioKey === 'factory' || i % 2 === 1 ? glassMaterial : bodyMaterial
...
function addIndustrialMachines(group: THREE.Group, color: number, secondary: number) {
  if (!activeScene.value.industrial) return

  const floor = new THREE.Mesh(
    new THREE.BoxGeometry(3.5, 0.035, 1.05),
    new THREE.MeshBasicMaterial({ color: 0x0f172a, transparent: true, opacity: 0.72 }),
  )
  floor.position.set(-0.18, -1.47, 0.72)
  group.add(floor)

  const conveyor = new THREE.Mesh(new THREE.BoxGeometry(2.72, 0.11, 0.22), mat(0x1f2937, color, 0.36))
  conveyor.position.set(-0.3, -1.3, 0.72)
  group.add(conveyor)

  for (let i = 0; i < 9; i += 1) {
    const roller = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.26, 12), new THREE.MeshBasicMaterial({ color: i % 2 ? color : secondary, transparent: true, opacity: 0.78 }))
    roller.rotation.x = Math.PI / 2
    roller.position.set(-1.55 + i * 0.31, -1.22, 0.72)
    group.add(roller)
    animated.push({ object: roller, mode: 'spin', speed: 0.035, offset: i })
  }

  for (let i = 0; i < 4; i += 1) {
    const part = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.14, 0.18), mat(i % 2 ? 0xf97316 : 0x38bdf8, secondary, 0.3))
    part.position.set(-1.35 + i * 0.62, -1.12, 0.72)
    group.add(part)
    animated.push({ object: part, mode: 'convey', speed: 0.0018, offset: i * 0.9 })
  }

  for (let i = 0; i < 3; i += 1) {
    const robotArm = new THREE.Group()
    robotArm.position.set(-1.18 + i * 1.05, -1.34, 0.22)
    const base = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.15, 0.14, 18), mat(0x111827, color, 0.42))
    const shoulder = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.42, 0.12), mat(0x334155, secondary, 0.36))
    shoulder.position.y = 0.28
    shoulder.rotation.z = i % 2 ? -0.35 : 0.35
    const forearm = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.38, 0.1), mat(0x475569, color, 0.36))
    forearm.position.set(i % 2 ? 0.17 : -0.17, 0.54, 0)
    forearm.rotation.z = i % 2 ? 0.7 : -0.7
    const tool = new THREE.Mesh(new THREE.SphereGeometry(0.055, 12, 8), new THREE.MeshBasicMaterial({ color: secondary }))
    tool.position.set(i % 2 ? 0.32 : -0.32, 0.72, 0)
    robotArm.add(base, shoulder, forearm, tool)
    group.add(robotArm)
    animated.push({ object: robotArm, mode: 'scan', speed: 0.002, offset: i })
  }

  const plcCabinet = new THREE.Group()
  plcCabinet.position.set(1.82, -1.06, 0.48)
  const cabinet = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.72, 0.18), mat(0x020617, color, 0.46))
  const statusLight = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.035), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.95 }))
  statusLight.position.set(0, 0.18, 0.094)
  const warningLight = new THREE.Mesh(new THREE.SphereGeometry(0.055, 16, 10), new THREE.MeshBasicMaterial({ color: 0xf97316, transparent: true, opacity: 0.95 }))
  warningLight.position.set(0, 0.46, 0)
  plcCabinet.add(cabinet, statusLight, warningLight)
  group.add(plcCabinet)
  animated.push({ object: statusLight, mode: 'pulse', speed: 0.005, offset: 0 })
  animated.push({ object: warningLight, mode: 'pulse', speed: 0.004, offset: 1 })

  const compressor = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.62, 24), mat(0x7c2d12, secondary, 0.36))
  compressor.rotation.z = Math.PI / 2
  compressor.position.set(1.56, -1.22, 1.22)
  group.add(compressor)

  addLine(group, new THREE.Vector3(1.82, -0.72, 0.48), new THREE.Vector3(0.25, -0.2, 0), color, 0.62)
  addLine(group, new THREE.Vector3(-0.3, -1.18, 0.72), new THREE.Vector3(0.25, -0.2, 0), secondary, 0.42)
  addLine(group, new THREE.Vector3(1.56, -1.22, 1.22), new THREE.Vector3(0.25, -0.2, 0), 0xf97316, 0.34)
}
...
function buildScene() {
  ...
  addIndustrialMachines(root, color, secondary)
...
function animate() {
  ...
  for (const item of animated) {
    ...
    if (item.mode === 'convey') {
      const t = (Math.sin(now * item.speed + item.offset) + 1) / 2
      item.object.position.x = -1.42 + t * 2.25
    }
    if (item.mode === 'scan') {
