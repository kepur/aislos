<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm ws-faint mt-1">{{ $t('pLeads.subtitle') }}</p>
      </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="ws-sunken/80 border-b ws-hairline">
            <th class="text-left px-4 py-3 text-xs font-semibold ws-faint uppercase tracking-wider">{{ $t('pLeads.colProjectType') }}</th>
            <th class="text-left px-4 py-3 text-xs font-semibold ws-faint uppercase tracking-wider">{{ $t('pLeads.colCountry') }}</th>
            <th class="text-left px-4 py-3 text-xs font-semibold ws-faint uppercase tracking-wider">{{ $t('pLeads.colBudget') }}</th>
            <th class="text-left px-4 py-3 text-xs font-semibold ws-faint uppercase tracking-wider">{{ $t('common.status') }}</th>
            <th class="text-left px-4 py-3 text-xs font-semibold ws-faint uppercase tracking-wider">{{ $t('pLeads.colSubmitted') }}</th>
          </tr>
        </thead>
        <tbody>
 ...
 class="border-b border-slate-50 hover:bg-blue-50/30 cursor-pointer transition-colors"
            ...
            <td class="px-4 py-3 ws-faint text-xs">{{ formatDay(lead.created_at) }}</td>
          </tr>
          <tr v-if="loading && !leads.length">
            <td colspan="5" class="px-4 py-12 text-center text-sm ws-faint">{{ $t('pLeads.loading') }}</td>
          </tr>
          <tr v-else-if="!leads.length">
            <td colspan="5" class="px-4 py-12 text-center">
              <div class="text-3xl mb-2">📋</div>
              <p class="ws-faint text-sm">{{ $t('common.noData') }}</p>
              <NuxtLink :to="localized('/submit-requirement')" class="inline-block mt-2 text-sm font-semibold ws-accent hover:opacity-80">{{ $t('pLeads.submitFirst') }}</NuxtLink>
            </td>
          </tr>
        </tbody>
      </table>
...
async function loadData() {
 ...
 leads.value = []
 error.value = e?.data?.detail || e?.message || t('pLeads.loadFailed')
  } finally {
 loading.value = false
  }
}
