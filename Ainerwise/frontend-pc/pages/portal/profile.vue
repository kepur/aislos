<template>
  <div class="space-y-6 max-w-2xl">
    ...
    <section class="portal-card space-y-5">
      <div>
        <h2 class="text-sm font-bold text-slate-800">Privacy & Data</h2>
        <p class="mt-1 text-sm text-slate-500">Export your account data or request account deletion. Financial, audit, order and project evidence may be retained after personal details are anonymized.</p>
      </div>
      ...
      <div v-if="privacyRequests.length" class="space-y-2">
        <article v-for="item in privacyRequests" :key="item.id" class="rounded-xl border border-slate-200 p-3 text-sm">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div><span class="font-semibold capitalize">{{ item.request_type }}</span> · {{ item.status }}</div>
            <div class="flex gap-2">
              <button v-if="item.request_type === 'export' && item.status === 'ready'" type="button" class="text-blue-600 hover:underline" @click="downloadExport(item.id)">Download</button>
          ...
          <p v-if="item.review_reason" class="mt-1 text-xs text-slate-500">{{ item.review_reason }}</p>
        </article>
      </div>
    </section>
  </div>
