  ...
  <section class="mx-auto max-w-3xl space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <h1 class="mt-1 text-2xl font-bold text-white">{{ $t('portal.team.title') }}</h1>
        <p class="mt-1 text-sm text-slate-400">{{ $t('portal.team.subtitle') }}</p>
      </div>
      <button class="btn-secondary" :title="inviteHint" @click="showInviteHint = !showInviteHint">{{ $t('portal.team.invite') }}</button>
    </div>

    <p v-if="showInviteHint" class="pc-card border-indigo-500/30 text-sm text-slate-300">{{ inviteHint }}</p>

    <div class="grid grid-cols-3 gap-4">
      <div class="pc-card"><p class="text-sm text-slate-400">{{ $t('portal.team.memberCount') }}</p><p class="mt-1 text-2xl font-bold text-white">{{ items.length }}</p></div>
      <div class="pc-card"><p class="text-sm text-slate-400">{{ $t('portal.team.activeCount') }}</p><p class="mt-1 text-2xl font-bold text-emerald-300">{{ activeCount }}</p></div>
    ...
    <div class="pc-card !p-0">
      ...
      <div v-else-if="!items.length" class="py-12 text-center text-slate-400">
        ...
        <p class="mt-2 text-sm">{{ $t('portal.team.noMembers') }}</p>
      </div>
      <div v-else>
        <div v-for="m in items" :key="m.id" class="flex items-center gap-4 border-b border-white/5 p-4 last:border-0">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-semibold text-white">{{ m.full_name || m.display_name || $t('portal.team.unnamed') }}</p>
          ...
          <span :class="['rounded-full px-2 py-0.5 text-xs', (m.is_active ?? m.status === 'ACTIVE') ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300']">{{ (m.is_active ?? m.status === 'ACTIVE') ? $t('portal.team.statusActive') : $t('portal.team.statusInactive') }}</span>
        </div>
      </div>
    </div>
...
const inviteHint = computed(() => t('portal.team.inviteHint'))
...
onMounted(async () => {
  try {
    ...
    error.value = e?.data?.detail || e?.message || t('portal.team.loadFailed')
  } finally {
    loading.value = false
  }
})
