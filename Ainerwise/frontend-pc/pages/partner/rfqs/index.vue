<template>
  <section class="space-y-5">
    ...
    <p v-if="!filtered.length&&!error" class="pc-card text-slate-500">No requests in this view.</p>
  </section>
</template>
...
const label=(v:string)=>v?.replaceAll('_',' ')||'unknown';const date=(v:string)=>v?new Date(v).toLocaleDateString():'not set';const location=(x:any)=>[x.scope_json?.city,x.scope_json?.country].filter(Boolean).join(', ')||'location pending'
