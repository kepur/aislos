  ...
  <section class="space-y-5">
    ...
    <template v-if="rfq">
      ...
      <div class="grid gap-5 lg:grid-cols-2">
        <div class="pc-card"><h2 class="font-semibold text-white">Scope facts</h2><dl class="mt-4 space-y-3 text-sm"><div v-for="item in scope" :key="item.key" class="border-b border-white/5 pb-3"><dt class="text-xs uppercase text-slate-500">{{ label(item.key) }}</dt><dd class="mt-1 text-slate-200">{{ item.value }}</dd></div></dl></div>
        <div v-if="rfq.bid" class="pc-card"><p class="text-xs uppercase tracking-wider text-emerald-300">Your submitted bid</p><p class="mt-3 text-3xl font-bold text-white">{{ rfq.bid.currency }} {{ Number(rfq.bid.amount).toLocaleString() }}</p><p class="mt-2 text-sm text-slate-400">{{ rfq.bid.lead_time_days??'—' }} days · {{ label(rfq.bid.status) }}</p><p class="mt-4 whitespace-pre-line text-sm text-slate-300">{{ rfq.bid.notes||'No notes.' }}</p></div>
        <form v-else-if="canRespond" class="pc-card space-y-4" @submit.prevent="submit"><h2 class="font-semibold text-white">Submit commercial bid</h2><div class="grid gap-3 sm:grid-cols-2"><input v-model.number="form.amount" required min="0.01" step="0.01" type="number" class="input-field" placeholder="Amount"><input v-model="form.currency" required maxlength="10" class="input-field" placeholder="Currency"></div><input v-model.number="form.lead_time_days" required min="0" type="number" class="input-field" placeholder="Lead time days"><textarea v-model="form.notes" rows="5" class="input-field" placeholder="Commercial and delivery notes"/><div class="flex gap-3"><button class="btn-primary" :disabled="saving">Submit bid</button><button type="button" class="btn-secondary" :disabled="saving" @click="decline">Decline</button></div></form>
      </div>
    </template>
  </section>
</template>
