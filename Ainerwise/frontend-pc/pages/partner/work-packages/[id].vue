    ...
    <template v-if="work">
      <div class="pc-card">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div><p class="text-xs uppercase tracking-wider text-blue-300">Work package</p><h1 class="mt-2 text-3xl font-bold text-white">{{ work.title }}</h1><p class="mt-2 text-sm text-slate-400">{{ label(work.trade || 'general delivery') }}</p></div>
          <span class="rounded-full bg-blue-500/10 px-3 py-1 text-xs uppercase text-blue-300">{{ label(work.status) }}</span>
        </div>
        <dl class="mt-5 grid gap-4 text-sm sm:grid-cols-3"><div><dt class="text-slate-500">Planned start</dt><dd class="mt-1 text-white">{{ work.planned_start || 'Not set' }}</dd></div><div><dt class="text-slate-500">Planned end</dt><dd class="mt-1 text-white">{{ work.planned_end || 'Not set' }}</dd></div><div><dt class="text-slate-500">Field tasks</dt><dd class="mt-1 text-white">{{ work.tasks.length }}</dd></div></dl>
      </div>

      <div v-for="task in work.tasks" :key="task.id" class="pc-card">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div><p class="text-xs uppercase text-blue-300">{{ label(task.task_type) }}</p><h2 class="mt-1 text-lg font-semibold text-white">{{ task.title }}</h2><p class="mt-2 text-xs text-slate-500">{{ task.required_skill || 'No skill restriction' }} · {{ task.evidence_count }} evidence records</p></div>
        ...
        <form class="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]" @submit.prevent="assign(task.id)">
          <select v-model="assignment[task.id].assignee_user_id" required class="input-field"><option value="">Select worker</option><option v-for="worker in workersForPackage" :key="worker.id" :value="worker.id">{{ worker.full_name || worker.email }}</option></select>
          <select v-model="assignment[task.id].crew_id" class="input-field"><option value="">No crew</option><option v-for="crew in crews" :key="crew.id" :value="crew.id">{{ crew.name }}</option></select>
          <button class="btn-primary py-2" :disabled="busy">Assign</button>
        </form>
        <button class="mt-4 text-sm text-blue-300" @click="loadEvidence(task.id)">View evidence</button>
        <div v-if="evidence[task.id]" class="mt-3 grid gap-2 md:grid-cols-2">
          ...
          <p v-if="!evidence[task.id].length" class="text-sm text-slate-500">No evidence submitted.</p>
        </div>
      </div>
    </template>
  </section>
