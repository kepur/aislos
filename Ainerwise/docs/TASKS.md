...
| PZL.27 | Procurement H5 buyer request list real empty state | READY_FOR_VERIFY | H5 `/buyer/requests` no longer falls back to hardcoded mock requests when Core returns an empty list; new buyer smoke returned `0` items, real intent creation made the list return exactly `1` Core item, unauthenticated list returned `401`, backend `py_compile`, and Procurement PC/H5/Admin builds pass |
| PZL.28 | Procurement PC Admin Users no-mock failure handling | READY_FOR_VERIFY | PC `/admin/users` no longer injects demo users when `/admin/users` fails and no longer pretends status toggles succeeded after failed API calls; failures now show a real error and clear stale rows; admin smoke read real user list, buyer access returned `403`, unauthenticated access returned `401`, backend `py_compile`, and Procurement PC/H5/Admin builds pass |
| PZL.29 | Procurement PC Admin Pricing Intelligence real analytics | READY_FOR_VERIFY | PC `/admin/pricing-intel` no longer renders chart placeholders or static benchmark anomalies; it loads real `/admin/offers` and `/admin/intents`, computes live offer/request metrics, status distributions, and budget variance signals with true empty states; admin smoke loaded 500 offers and 500 intents from current dev data, buyer access to both APIs returned `403`, unauthenticated admin offers returned `401`, backend `py_compile`, and Procurement PC/H5/Admin builds pass |
| PZL.30 | Procurement PC Admin Offers and Intents real lists | READY_FOR_VERIFY | PC `/admin/offers` and `/admin/intents` no longer render hardcoded `OFF-101`/`INT-82910` sample rows; both pages load real Core `/admin/offers` and `/admin/intents`, support search/status filtering, show true empty/error states, expose real detail panels, and call real moderation endpoints for offer removal and intent flag/cancel/restore; admin smoke read both APIs, buyer access returned `403`, unauthenticated access returned `401`, static sample scans returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.31 | Procurement PC Supplier Offers real list | READY_FOR_VERIFY | PC `/supplier/offers` no longer renders hardcoded supplier offer rows; it uses the shared offer store backed by Core `/supplier/offers`, supports search/status filtering, true empty/error states, real detail panels, and real `/offers/{id}/withdraw` for active offers; smoke created a supplier company, catalog item, buyer intent, supplier offer, confirmed the created offer appears in supplier list, buyer access returned `403`, unauthenticated access returned `401`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.32 | Procurement PC Supplier Orders real list | READY_FOR_VERIFY | PC `/supplier/orders` no longer renders the hardcoded `John Doe Construction / 500 bags Portland Cement` sample order; it uses the shared order store backed by Core `/orders/my`, supports search/status filtering, true empty/error states, and links to real order detail ids; smoke created a direct marketplace order, confirmed it appears for the supplier company, confirmed another supplier cannot see it, unauthenticated `/orders/my` returned `401`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.33 | Procurement PC Supplier Order Detail real delivery workspace | READY_FOR_VERIFY | PC `/supplier/orders/[id]` no longer renders the hardcoded `ORD-82910 / Portland Cement / Oceanic Transport` detail; it loads real Core `/orders/{id}` through the shared order store, displays real order/escrow/delivery fields, writes real `/orders/{id}/delivery` updates, and preserves real buyer-review submission; smoke created a direct order, supplier read detail returned `200`, another supplier returned `403`, unauthenticated detail returned `401`, supplier delivery update returned `201` and tracking number was visible after reload, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.34 | Procurement PC Supplier Dashboard real aggregates | READY_FOR_VERIFY | PC `/supplier/dashboard` no longer renders hardcoded pings, fake KPI numbers, or static action rows; it loads real Core `/supplier/intents/matching`, `/supplier/offers`, and `/orders/my` through shared stores, computes pings/active offers/awarded/pending delivery/escrow payout from source data, uses `N/A` for unconnected SLA/rating/dispute metrics, and shows true loading/error/empty states; smoke verified created offer/order appear in supplier source APIs, other supplier cannot see the order, buyer supplier-matching access returned `403`, unauthenticated supplier offers returned `401`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.35 | Procurement H5 Supplier Ads real campaign workflow | READY_FOR_VERIFY | H5 `/supplier/ads` no longer falls back to mock campaign rows and H5 `/supplier/ads/create` no longer simulates success when the API fails; campaign list/create/submit/pause now rely on real Core `/merchant/ad-campaigns*` responses, API errors show real failure states, and empty state is genuine; smoke created a real supplier campaign, listed it, submitted it to `PENDING_REVIEW`, paused it to `PAUSED`, buyer list/create returned `403`, unauthenticated list returned `401`, static mock scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.36 | Procurement H5 Supplier Catalog real CRUD | READY_FOR_VERIFY | H5 `/supplier/catalog` no longer falls back to `MOCK_ITEMS`, sample products, or fake `mi*` local item actions; catalog list/statistics/search read only real Core `/supplier/catalog/items`, API errors show a real retry state, and create/update/status/delete all require real backend success; smoke created a real item, listed it, patched price/stock, deactivated it, verified other supplier patch `404`, buyer list `403`, unauthenticated list `401`, delete `204`, deleted read `404`, static mock scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.37 | Procurement PC/H5 Marketplace no demo fallback | READY_FOR_VERIFY | PC/H5 `/marketplace` and `/marketplace/[id]` no longer import `demoMarketplaceItems` or fill failed feed/detail calls with demo products; failed feed calls show a real retry error state, failed detail calls remain `Product not found`, and public feed/detail use only Core `/marketplace/feed` and `/marketplace/items/{id}`; smoke created a real supplier catalog item, verified public feed contains it, detail returns it, missing item returns `404`, static demo scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.38 | Procurement H5 Buyer Offers no demo fallback | READY_FOR_VERIFY | H5 `/buyer/offers`, `/buyer/requests/[id]`, and `/buyer/compare` no longer import or call `demoOffersForIntent`; the shared H5 intent store clears offers and exposes a real `offerError` on API failure instead of creating demo offers; smoke created a real supplier catalog item, buyer intent, supplier offer, verified buyer offer list contains the real offer, unrelated buyer returned `403`, unauthenticated list returned `401`, static demo-offer scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.39 | Procurement PC Buyer Dashboard real aggregates | READY_FOR_VERIFY | PC `/buyer/dashboard` no longer renders hardcoded active request rows, fake KPI counts, fake escrow, fake recent-message companies, or static recommended categories; it reads real Core `/intents/my`, `/orders/my`, `/marketplace/feed`, and account/trust APIs, computes buyer KPIs from source data, and shows true empty/error states for unconnected messages/categories; smoke verified real buyer intent appears in dashboard source API, orders source returns `200`, unauthenticated intent/order sources return `401`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.40 | Procurement PC Buyer Request Detail real candidates | READY_FOR_VERIFY | PC `/buyer/requests/[id]` no longer imports `demoIntents`, `demoSupplierCandidatesForIntent`, or demo binding logic; request detail failures show a real unavailable state, supplier candidates consume Core `items`/`candidates` from `/intents/{id}/supplier-candidates`, detail and bind use real Core endpoints, and API failures show retryable errors; smoke verified candidate list/detail/bind `200`, created catalog item appears in candidates, unrelated buyer returned `403`, unauthenticated candidates returned `401`, static demo scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.41 | Procurement PC Buyer Offer Detail real page | READY_FOR_VERIFY | PC `/buyer/offers/[id]` no longer renders Cebu Builders Hub / Portland Cement / `$2,150` placeholder data; it loads real Core `/offers/{id}` through `useApi.getOffer`, shows real supplier/price/logistics/escrow terms, and awards through `/offers/{id}/award` to a real order id; smoke created a buyer/supplier/catalog/intent/offer chain, buyer offer detail returned `200`, unrelated buyer returned `403`, unauthenticated detail returned `401`, award returned `200` and created order `fa56b95b-3386-4ef1-a85b-5a24623c7f41`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
| PZL.42 | Procurement PC Admin Categories real taxonomy list | READY_FOR_VERIFY | PC `/admin/categories` no longer renders hardcoded `CAT-101` / Portland Cement / Marine Engines / Office Laptops rows; it loads real Core `/categories`, supports search, loading/error/empty states, shows schema field counts from `schema_json`, disables unimplemented write actions instead of faking add/edit, and adds the missing PC `admin` route middleware used by admin pages; smoke logged in as admin, fetched `1333` Core categories with first id `5d2a4c6f-bac4-4247-bab2-43afff9a9220`, static sample scan returned no matches, and Procurement PC/H5/Admin builds pass |
