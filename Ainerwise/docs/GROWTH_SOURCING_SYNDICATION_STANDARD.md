...
- **P3（自动化）✅（定时重算）**：`reprice_due_growth_rules` celery beat 任务（每日 05:15 UTC，automation 队列）重算所有 active 非 manual 规则，fx 自动走 `ExchangeRate` 表，超阈值才改价——已 E2E 验证（daily 规则 + 表内 CNY→EUR 0.14 → 自动重算 €20.00）。**待做**：LLM 端到端编排（`TOOL_SIGNATURES` → `agent_team` 挂工具，让 agent 自己跑采集→翻译→定价→出图文→排期）。
