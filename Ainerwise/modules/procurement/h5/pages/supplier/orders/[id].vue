  ...
  <div class="min-h-screen bg-slate-50 flex flex-col">
    ...
    <template v-else-if="order">
      <div class="flex-1 overflow-y-auto pb-40">
        ...
        <div v-if="order.escrow" class="card mx-4 mt-3">
          <div class="flex items-center gap-2 mb-3">
            ...
            <h3 class="font-semibold text-slate-900">{{ $t("pages.escrow") }}</h3>
        ...
        <div v-if="order.notes" class="card mx-4 mt-3">
          <p class="text-xs text-slate-500 font-medium mb-1">Buyer Notes</p>
      ...
      <div class="sticky-bottom-action space-y-2">
        <!-- Mark as shipped -->
        <button type="button"
          v-if="order.status === 'PAID_IN_ESCROW' || order.status === 'IN_PROGRESS'"
          class="btn-primary"
          :disabled="actionLoading"
          @click="openShipSheet = true"
        >
        ...
        <NuxtLink v-if="order.status !== 'CANCELED'" :to="`/messages/${order.id}`">
          <button type="button" class="btn-secondary py-3 text-sm">Message Buyer</button>
        </NuxtLink>
      </div>

      <!-- Ship Sheet -->
      <van-action-sheet v-model:show="openShipSheet" title="Update Delivery">
        <div class="px-4 pb-6 pt-2 space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1.5">Status</label>
            <select v-model="deliveryStatus" class="input-field bg-white">
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1.5">Tracking Number (optional)</label>
            <input v-model="trackingNumber" type="text" class="input-field" placeholder="e.g. JRS123456" />
          </div>
          <button type="button"
            class="btn-primary"
            :disabled="!deliveryStatus || actionLoading"
...
async function updateDelivery() {
  ...
  try {
    await orderStore.createDelivery(id, deliveryStatus.value, trackingNumber.value || undefined);
    openShipSheet.value = false;
    showToast({ type: "success", message: "Delivery status updated." });
