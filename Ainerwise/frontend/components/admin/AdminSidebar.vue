  ...
  >
    ...
    <div class="px-5 py-5 border-b border-white/5">
      <NuxtLink to="/" class="flex items-center gap-3">
        ...
        <div>
          <span class="text-base font-bold text-white tracking-wide">AinerWise</span>
          <span class="block text-[10px] text-cyan-400/80 font-medium tracking-widest uppercase">Command Center</span>
        </div>
      </NuxtLink>
    </div>

    <!-- Navigation -->
    <nav class="flex-1 py-4 px-3 space-y-6 overflow-y-auto scrollbar-thin">
      <template v-for="section in menuSections" :key="section.title">
        <div>
          <p class="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-2 px-3">{{ section.title }}</p>
...
const menuSections = computed(() => [
  {
    title: 'Overview',
    items: [
      { to: '/admin', label: t('admin.dashboard'), icon: IconDashboard },
    ],
  },
  {
    title: 'Business',
    items: [
      { to: '/admin/leads', label: t('admin.leads'), icon: IconLeads },
      { to: '/admin/projects', label: t('admin.projects'), icon: IconProject },
      { to: '/admin/quotes', label: t('admin.quotes'), icon: IconQuote },
      { to: '/admin/proposals', label: t('admin.proposals'), icon: IconProposal },
      { to: '/admin/tickets', label: t('admin.tickets'), icon: IconTicket },
      { to: '/admin/inquiries', label: 'Inquiries', icon: IconInquiry },
    ],
  },
  {
    title: 'Catalog',
    items: [
      { to: '/admin/products', label: t('admin.products'), icon: IconProduct },
      { to: '/admin/categories', label: t('admin.categories'), icon: IconCategory },
      { to: '/admin/solutions', label: t('admin.solutions'), icon: IconSolution },
      { to: '/admin/service-packages', label: t('admin.servicePackages'), icon: IconPkg },
      { to: '/admin/compatibility', label: t('admin.compatibility'), icon: IconLink },
      { to: '/admin/warranty-policies', label: t('admin.warrantyPolicies'), icon: IconShield },
    ],
  },
  {
    title: 'Network',
    items: [
      { to: '/admin/vendors', label: t('admin.vendors'), icon: IconVendor },
      { to: '/admin/service-partners', label: t('admin.servicePartners'), icon: IconPartner },
    ],
  },
  {
    title: 'Users',
    items: [
      ...
      { to: '/admin/companies', label: t('admin.companies'), icon: IconCompany },
    ],
  },
  {
    title: 'System',
    items: [
      { to: '/admin/ai-runs', label: t('admin.aiRuns'), icon: IconAI },
      { to: '/admin/integration-events', label: t('admin.events'), icon: IconEvent },
      { to: '/admin/certifications', label: t('admin.certifications'), icon: IconCert },
      { to: '/admin/regions', label: t('admin.regions'), icon: IconGlobe },
      { to: '/admin/audit-logs', label: t('admin.auditLogs'), icon: IconAudit },
      { to: '/admin/settings', label: t('admin.settings'), icon: IconGear },
    ],
  },
])
</script>
