/** Shared Commerce API client (Phase 3 — Core-owned trade domain). */
export function useCommerce() {
  ...
  return {
    ...
    getProcurementRequest: (id: string) => apiFetch<any>(`${prefix}/procurement-requests/${id}`),
    createProcurementRequest: (body: Record<string, unknown>) =>
      apiFetch<any>(`${prefix}/procurement-requests`, { method: 'POST', body: { portal_key: 'cebu', ...body } }),
    publishRequest: (id: string) =>
      apiFetch<any>(`${prefix}/procurement-requests/${id}/publish`, { method: 'POST' }),
    listCandidates: (id: string) =>
      apiFetch<{ items: any[]; total: number }>(`${prefix}/procurement-requests/${id}/supplier-candidates`),
    ...
    unreadCount: () => apiFetch<{ count: number }>(`${prefix}/notifications/unread-count`),
    markRead: (id: string) =>
      apiFetch(`${prefix}/notifications/${id}/read`, { method: 'POST' }),
    markAllRead: () => apiFetch(`${prefix}/notifications/read-all`, { method: 'POST' }),
    getOrder: (id: string) => apiFetch(`${prefix}/orders/${id}`),
    ...
    listDisputes: () => apiFetch<{ items: any[]; total: number }>(`${prefix}/disputes`),
    ...
    getOrderThread: (id: string) => apiFetch(`${prefix}/orders/${id}/thread`),
