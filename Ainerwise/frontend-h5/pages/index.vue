<template>
  <div class="px-4 py-4 space-y-5">
    ...
    <div class="hero-card rounded-2xl p-5 text-white relative overflow-hidden">
      <div class="relative z-10">
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
          <NuxtLink to="/products" class="text-center text-xs font-bold bg-white text-blue-600 px-4 py-2.5 rounded-full shadow-sm">
            {{ $t('home.heroCtaProducts') }}
          </NuxtLink>
          <a :href="marketH5Url" class="text-center text-xs font-bold bg-emerald-400 text-slate-950 px-4 py-2.5 rounded-full shadow-sm">
            {{ $t('home.heroCtaMarket') }}
          </a>
        </div>
        <NuxtLink to="/submit-requirement" class="mt-2 inline-flex text-[11px] font-semibold text-blue-100 underline decoration-white/30 underline-offset-4">
          {{ $t('home.heroCtaAssessment') }}
        </NuxtLink>
      </div>
      <!-- Decorative circles -->
    ...
    <div class="rounded-3xl border border-blue-100 bg-white p-4 shadow-sm">
      <p class="text-[10px] font-bold uppercase tracking-wider text-blue-500">{{ $t('home.shopKicker') }}</p>
      <h2 class="mt-1 text-lg font-extrabold text-slate-900">{{ $t('home.shopTitle') }}</h2>
      <p class="mt-1 text-xs leading-relaxed text-slate-500">{{ $t('home.shopSubtitle') }}</p>
      <div class="mt-4 grid grid-cols-3 gap-2">
        <NuxtLink to="/products" class="rounded-2xl border border-blue-100 bg-blue-50 p-3 active:bg-blue-100">
          <span class="block text-lg">🛒</span>
          <span class="mt-1 block text-[11px] font-extrabold text-blue-700">{{ $t('home.shopOfficial') }}</span>
          <span class="mt-0.5 block text-[10px] leading-4 text-blue-500">{{ $t('home.shopOfficialDesc') }}</span>
        </NuxtLink>
        <a :href="marketH5Url" class="rounded-2xl border border-emerald-100 bg-emerald-50 p-3 active:bg-emerald-100">
          <span class="block text-lg">🏬</span>
          <span class="mt-1 block text-[11px] font-extrabold text-emerald-700">{{ $t('home.shopMarket') }}</span>
          <span class="mt-0.5 block text-[10px] leading-4 text-emerald-600">{{ $t('home.shopMarketDesc') }}</span>
        </a>
        <a :href="`${marketH5Url}/secondhand`" class="rounded-2xl border border-slate-200 bg-slate-50 p-3 active:bg-slate-100">
          <span class="block text-lg">♻️</span>
          <span class="mt-1 block text-[11px] font-extrabold text-slate-800">{{ $t('home.shopSecondhand') }}</span>
          <span class="mt-0.5 block text-[10px] leading-4 text-slate-500">{{ $t('home.shopSecondhandDesc') }}</span>
        </a>
      </div>
    </div>
