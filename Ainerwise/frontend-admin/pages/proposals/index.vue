<template>
  <div>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
      <div>
        <h1 class="admin-page-title">Proposal Plans</h1>
        <p class="admin-page-desc">Four-tier smart building proposals with cost breakdown.</p>
      </div>
      ...
      <table class="admin-table w-full text-sm">
        ...
        <tbody>
          <tr v-for="p in proposals" :key="p.id" class="border-b">
            ...
            <td class="px-4 py-3">
              <NuxtLink v-if="p.lead_id" :to="`leads/${p.lead_id}`" class="text-primary-600 hover:underline text-xs">
                L-{{ p.lead_id.slice(0, 6) }}
              </NuxtLink>
              <NuxtLink v-if="p.project_id" :to="`projects/${p.project_id}`" class="text-blue-600 hover:underline text-xs ml-1">
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-1">
                <NuxtLink :to="`proposals/${p.id}`" class="px-2 py-1 text-xs font-medium rounded bg-blue-100 text-blue-700 hover:bg-blue-200">
                  BOM
                </NuxtLink>
