<template>
  <div>
    ...
    <h1 class="admin-page-title mt-4 mb-6">Create Solution</h1>
    ...
    <form class="max-w-3xl space-y-6" @submit.prevent="handleSubmit">
      ...
      <div class="admin-card space-y-4">
        <h2 class="admin-section-title">Details (comma-separated lists)</h2>
        <div>
      ...
      <div class="flex gap-3">
        ...
        <NuxtLink to="solutions" class="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</NuxtLink>
      </div>
    </form>
  </div>
</template>
...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = e?.data?.detail || e?.message || 'Solution could not be created'
  } finally {
    loading.value = false
  }
}
