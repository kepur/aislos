<template>
  <button
    type="button"
    class="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition flex items-center justify-center"
    :title="isLight ? 'Switch to dark mode' : 'Switch to light mode'"
    :aria-label="isLight ? 'Switch to dark mode' : 'Switch to light mode'"
    @click="toggleTheme"
  >
    <svg v-if="isLight" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
