...
| Core API Migration | Replace transitional Cebu API with AinerWise Core compatible API | IN_PROGRESS | Core bridge V1 is READY_FOR_VERIFY for auth, users, categories, marketplace feed/detail without demo fallback, payment region config, buyer intents, supplier offers/orders/notifications adapters, RFQ / Intent / Offer workflow, PC buyer dashboard real aggregates, PC buyer request detail real supplier candidates, PC buyer offer detail/award real order flow, PC supplier inbox/offer submission/list/dashboard aggregates, PC supplier orders list/detail delivery workspace, H5 buyer request list/detail candidates/offers plus H5 buyer offers/compare without demo offer fallback, wallet/deposit compatibility, Buyer Projects / AI Project Forge main chain, Orders / Escrow / Delivery compatibility, shipping estimate compatibility, direct marketplace order creation, buyer profile/team, generic uploads, trust profile, account context/type, address book, ranking profiles, order message-thread compatibility, Supplier Catalog / Ads compatibility including H5 real catalog and campaign workflows, Dispute / KYC / Risk compatibility, Admin read-panel gap bridge, Admin AI/Maps config bridge, Admin action compatibility bridge, Admin audit/notifications/backups bridge, and PC Admin Categories real Core taxonomy list; full admin parity still requires ledger gates |
| Ledger-Based Migration | Migrate Marketplace, Project Forge, RFQ, Order, Wallet, Message, KYC, Dispute, Admin panels | IN_PROGRESS | Legacy UI copied; public marketplace/auth/intent bridge verified with no PC/H5 demo product fallback; RFQ/Intent/Offer, PC Buyer Dashboard/Request Detail/Offer Detail, PC Supplier Inbox/Offer Submit/List/Dashboard, PC Supplier Orders List/Detail, H5 Buyer Request List/Detail and Buyer Offers/Compare no-demo-offer fallback, H5 Supplier Catalog/Ads, Profile/Upload/Direct Order, Address/Ranking, Shipping Estimate, and Dispute/KYC/Risk workflows are READY_FOR_VERIFY; Admin read-panel, AI/Maps config, action, audit, notifications, backups, PC Admin Users no-mock handling, PC Admin Pricing Intelligence real analytics, PC Admin Offers/Intents real lists, and PC Admin Categories real taxonomy list are READY_FOR_VERIFY; remaining admin workflows continue module by module |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Public marketplace | PC/H5 | `/marketplace`, `/categories` | Guest, Buyer, Supplier | Browse categories/products/suppliers, direct Buy Now | `Ainerwise/modules/procurement/pc`, `h5` | Core `/api/v1/cebu-compat/categories`, `/marketplace/feed`, `/marketplace/items/*`, `/orders` | READY_FOR_VERIFY for UI copy, public Core read/detail bridge without demo fallback, PC Admin Categories read-only Core taxonomy list, and direct Core order creation | `curl -X POST http://procurement.localhost/api/orders -H "Authorization: Bearer $buyer_token" -d '{"catalog_item_id":"...","qty":2}'` | `HTTP 201`, creates Core `procurement_requests`, `supplier_offers`, and `commerce_orders`; PC/H5 marketplace feed/detail no longer inject demo products when Core fails; PC `/admin/categories` no longer shows hardcoded `CAT-101` sample rows and reads `/categories`; unrelated buyer read returns `403` |
...
| RFQ / Intent / Offer | PC/H5/Admin | `/intents`, `/offers`, `/requests` | Buyer, Supplier, Admin | Request, match supplier, submit offer, award | `Ainerwise/modules/procurement/*` | Core `/api/v1/cebu-compat/intents`, `/intents/*/offers`, `/offers/*`, `/supplier/intents/matching`, `/supplier/offers`, `/intents/*/supplier-candidates`, `/admin/intents/*`, `/admin/offers/*` | READY_FOR_VERIFY for create/publish/match/offer/detail/withdraw/award/admin moderation bridge plus PC buyer dashboard/request detail/offer detail, PC supplier inbox/offer submission/list/dashboard, and H5 buyer request list/detail/offers/compare UI chains without demo offer fallback | `curl -X POST http://procurement.localhost/api/intents -H "Authorization: Bearer $buyer_token" -d @intent.json` | Buyer create/publish, supplier matching, PC buyer dashboard consumes real `/intents/my` and `/orders/my` sources with no hardcoded KPI/request rows, PC buyer request detail consumes real Core `items`/`candidates` and no longer injects demo intent/candidates/bind state, PC buyer offer detail consumes real `/offers/{id}` and awards through `/offers/{id}/award` to a real order id, PC inbox consumes real pings, PC offer editor loads intent and submits real offer, PC supplier offers list consumes real `/supplier/offers` with no static sample rows, PC supplier dashboard consumes real matching pings/offers/order source APIs with no fake KPI rows, H5 buyer request list uses true Core empty state and real created intents, H5 buyer request detail consumes real candidate `items` and no longer injects mock offers/candidates, H5 buyer offers and compare no longer inject demo offers when Core fails, buyer offer detail, cross-supplier 403, unrelated buyer offer detail 403, unauthenticated offer detail 401, buyer role denied supplier matching/list 403, unauthenticated request/supplier-offer/order/candidate source APIs return 401, unrelated buyer offer submit/list/candidate read returns 403, cross-buyer candidate read/bind 403, duplicate supplier offer denied, supplier withdraw, admin intent flag, admin offer remove, buyer award to order, supplier admin 403, PC/H5/Admin builds pass |
| Orders / Escrow / Wallet | PC/H5/Admin | `/orders`, `/wallet`, admin payments/escrow | Buyer, Supplier, Finance/Admin | Create order, escrow, delivery, accept, release, reviews, payout wallet credit | `Ainerwise/modules/procurement/*` | Core `/api/v1/cebu-compat/orders/*`, `/api/v1/cebu-compat/wallets/*`, and admin `/api/v1/cebu-compat/admin/{escrow,deposits,payouts,payment-events,settlement-events,payment-region-configs}` | READY_FOR_VERIFY for direct marketplace order creation, PC Supplier Dashboard/Orders real list/detail delivery workspace, Wallet/Deposit, Orders/Escrow/Delivery, and admin payment read panels; deeper payout operations remain IN_PROGRESS | `curl -X POST http://procurement.localhost/api/orders/$order_id/pay-from-wallet -H "Authorization: Bearer $buyer_token"` | Direct Buy Now creates Core order, PC Supplier Dashboard and Supplier Orders list consume real `/orders/my` with no static sample row, PC Supplier Order Detail consumes real `/orders/{id}` and writes `/orders/{id}/delivery`, wallet deposit/admin verify, wallet payment, escrow capture, supplier delivery, buyer acceptance, escrow release, supplier wallet credit, buyer/supplier reviews, admin payment read-panel smoke, and cross-supplier/customer 403 verified |
...
| Supplier Catalog / Ads | PC/H5/Admin | `/catalog`, `/ads`, admin campaigns | Supplier, Admin | Manage products/services, supplier ads | `Ainerwise/modules/procurement/*` | Core `/api/v1/cebu-compat/supplier/catalog/items`, `/api/v1/cebu-compat/merchant/ad-campaigns`, `/api/v1/cebu-compat/admin/ad-campaigns` | READY_FOR_VERIFY for supplier catalog CRUD, H5 real catalog CRUD UI, merchant ad campaign workflow, H5 real campaign list/create/submit/pause UI, and admin campaign review bridge | `curl -X POST http://procurement.localhost/api/supplier/catalog/items -H "Authorization: Bearer $supplier_token" -d @catalog.json` | Supplier create/list/update/delete, H5 supplier catalog no longer falls back to mock product inventory or fake local item actions, campaign create/submit/admin approve/supplier pause, H5 supplier ads no longer falls back to mock campaigns or simulated success, buyer catalog/campaign access 403, unauthenticated catalog/campaign access 401, cross-supplier catalog 404, cross-supplier campaign 404, PC/H5/Admin builds pass |
...
### Public Marketplace No-Demo-Fallback Evidence
...
| Check | Command | Result |
| --- | --- | --- |
| Marketplace demo fallback removal scan | `rg -n "demoMarketplace\|demoData\|Cement Summer Sale\|Portland Cement Type I\|Feed error.*demo\|demoItem" Ainerwise/modules/procurement/pc/pages/marketplace Ainerwise/modules/procurement/h5/pages/marketplace -g '*.vue'` | No matches |
| Public feed returns real item | Register supplier with `company_name`, create active `/supplier/catalog/items` titled `PZL37 Real Marketplace Item`, then `GET http://procurement.localhost/api/marketplace/feed?keyword=PZL37%20Real%20Marketplace%20Item&page_size=10` | `HTTP 200`; feed contains created Core item `6525b74c-b16a-41d2-8e18-5a975878782a` |
| Public detail returns real item | `GET http://procurement.localhost/api/marketplace/items/6525b74c-b16a-41d2-8e18-5a975878782a` | `HTTP 200`; returned id/title match created item |
| Missing public detail stays missing | `GET http://procurement.localhost/api/marketplace/items/00000000-0000-4000-8000-000000000000` | `404`; UI no longer has a demo item fallback path |
...
### PC Admin Categories Real Taxonomy Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| Modified files | `git diff --name-only -- Ainerwise/modules/procurement/pc/pages/admin/categories.vue Ainerwise/modules/procurement/pc/middleware/admin.ts Ainerwise/docs/TASKS.md AINERWISE_PROCUREMENT_ZERO_LOSS_EXECUTION.md` | PC Admin Categories page, missing PC admin route middleware, and task ledgers only |
| Page address | `http://procurement.localhost/admin/categories` | Standalone PC Procurement admin taxonomy route; guarded by `middleware/admin.ts` |
| Static category sample removal scan | `rg -n "CAT-101\|CAT-102\|CAT-103\|Portland Cement\|Marine Engines\|Office Laptops\|const categories = \\[\|suppliers: 142\|Registered Suppliers" Ainerwise/modules/procurement/pc/pages/admin/categories.vue` | No matches |
| Admin categories source API | Login `admin@ainerwise.com / admin123456`, then `GET http://procurement.localhost/api/categories -H "Authorization: Bearer $admin_token"` | `HTTP 200`, returned `1333` Core categories; first id `5d2a4c6f-bac4-4247-bab2-43afff9a9220`, first name `Cebu Demo Lighting`, status `ACTIVE` |
| Route middleware build gate | `Ainerwise/modules/procurement/pc/middleware/admin.ts` | Existing PC pages using `middleware: ['admin']` now resolve to a real middleware file; non-admin users navigate away from admin routes client-side |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |
...
### H5 Supplier Ads Real Campaign Workflow Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| H5 ads mock removal scan | `rg -n "Mock campaigns\|MOCK_CAMPAIGNS\|demo mode\|simulate success\|mc1\|Cement Summer Sale\|Medical Supplies Promo\|Safety Equipment Q4\|Office Furniture Blowout\|startsWith\\('mc'\\)" Ainerwise/modules/procurement/h5/pages/supplier/ads` | No matches |
| Supplier creates real campaign | Register supplier with `company_name`, then `POST http://procurement.localhost/api/merchant/ad-campaigns -H "Authorization: Bearer $supplier_token" -d '{"name":"PZL35 H5 Real Campaign","placement":"FEED_TOP","budget_minor":250000,"bid_per_click_minor":500,"currency":"PHP"}'` | `HTTP 201`, real Core campaign `a05e3eb8-fdb1-443e-909b-7606f545c184`, status `DRAFT` |
| Supplier lists campaign | `GET http://procurement.localhost/api/merchant/ad-campaigns -H "Authorization: Bearer $supplier_token"` | `HTTP 200`; list contains created campaign id |
| Supplier submits campaign | `POST http://procurement.localhost/api/merchant/ad-campaigns/a05e3eb8-fdb1-443e-909b-7606f545c184/submit -H "Authorization: Bearer $supplier_token"` | `HTTP 200`, status `PENDING_REVIEW` |
| Supplier pauses campaign | `POST http://procurement.localhost/api/merchant/ad-campaigns/a05e3eb8-fdb1-443e-909b-7606f545c184/pause -H "Authorization: Bearer $supplier_token"` | `HTTP 200`, status `PAUSED` |
| Buyer denied campaign list/create | `GET` and `POST http://procurement.localhost/api/merchant/ad-campaigns` with buyer token | Both returned `403` |
| Unauthenticated campaign denial | `GET http://procurement.localhost/api/merchant/ad-campaigns` | `401` |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |

### H5 Supplier Catalog Real CRUD Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| H5 catalog mock removal scan | `rg -n "MOCK_ITEMS\|mi1\|Portland Cement Type I\|Steel Rebar\|Nitrile Gloves\|Office Chair\|Marine Plywood\|demo mode\|startsWith\\('mi'\\)\|picsum\\.photos/seed/20" Ainerwise/modules/procurement/h5/pages/supplier/catalog.vue` | No matches |
| Supplier creates real catalog item | Register supplier with `company_name`, then `POST http://procurement.localhost/api/supplier/catalog/items -H "Authorization: Bearer $supplier_token" -d '{"title":"PZL36 H5 Real Catalog Item","price_minor":123400,"currency":"PHP","status":"ACTIVE","unit":"set","stock_qty":8,"market_mode":"B2B"}'` | `HTTP 201`, real Core item `b82e8c97-7c9e-4572-94eb-24b28bb9181f` |
| Supplier lists real item | `GET http://procurement.localhost/api/supplier/catalog/items?keyword=PZL36%20H5%20Real%20Catalog%20Item&page_size=5 -H "Authorization: Bearer $supplier_token"` | `HTTP 200`; list contains created item id |
| Supplier patches item | `PATCH http://procurement.localhost/api/supplier/catalog/items/b82e8c97-7c9e-4572-94eb-24b28bb9181f -H "Authorization: Bearer $supplier_token" -d '{"price_minor":130000,"stock_qty":3}'` | `HTTP 200`, price and stock updated |
| Supplier deactivates item | `PATCH http://procurement.localhost/api/supplier/catalog/items/b82e8c97-7c9e-4572-94eb-24b28bb9181f -H "Authorization: Bearer $supplier_token" -d '{"status":"INACTIVE"}'` | `HTTP 200`, status `INACTIVE` |
| Other supplier denied item patch | Same item patch with other supplier token | `404` |
| Buyer denied supplier catalog list | `GET http://procurement.localhost/api/supplier/catalog/items -H "Authorization: Bearer $buyer_token"` | `403` |
| Unauthenticated catalog denial | `GET http://procurement.localhost/api/supplier/catalog/items` | `401` |
| Supplier deletes item | `DELETE http://procurement.localhost/api/supplier/catalog/items/b82e8c97-7c9e-4572-94eb-24b28bb9181f -H "Authorization: Bearer $supplier_token"` then `GET` same item | `204`; later read returned `404` |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |
...
### H5 Buyer Offers No-Demo-Fallback Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| H5 demo offer removal scan | `rg -n "demoOffersForIntent\|demo-offer\|fake offer\|mock offer\|isDemoToken\\(authStore\\.accessToken\\).*offer" Ainerwise/modules/procurement/h5/stores/intent.ts Ainerwise/modules/procurement/h5/pages/buyer/offers.vue 'Ainerwise/modules/procurement/h5/pages/buyer/requests/[id].vue' Ainerwise/modules/procurement/h5/pages/buyer/compare.vue` | No matches |
| H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Real buyer offer chain | Register buyer/vendor/other buyer, create vendor company, create active catalog item, create buyer intent with a real `/categories` id, publish intent, submit supplier offer, then `GET http://procurement.localhost/api/intents/27569e3d-fa60-4ef6-8737-a4ea008acb65/offers -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, buyer-visible offers include real offer `0b89bd35-d509-42e4-8bbd-4568b8916389` |
| Cross-buyer offer list denial | Same offer list with unrelated buyer token | `403` |
| Unauthenticated offer list denial | Same offer list without token | `401` |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |

### PC Buyer Dashboard Real Aggregates Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| Static dashboard sample removal scan | `rg -n "500 bags Portland Cement\|Office Laptops\|Marine Engine Parts\|Global Build Supply Co\|Construction</UBadge>\|Steel Rebar\|Heavy Machinery\|\\$4,500\|const requests = computed\\(\\(\\) => \\[" Ainerwise/modules/procurement/pc/pages/buyer/dashboard.vue` | No matches |
| Buyer dashboard source intents | `GET http://procurement.localhost/api/intents/my?page=1&page_size=20 -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, returned `1` real item and included intent `27569e3d-fa60-4ef6-8737-a4ea008acb65` |
| Buyer dashboard source orders | `GET http://procurement.localhost/api/orders/my?page=1&page_size=20 -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, returned a real empty order list for the smoke buyer |
| Unauthenticated intent source denial | `GET http://procurement.localhost/api/intents/my?page=1&page_size=20` | `401` |
| Unauthenticated order source denial | `GET http://procurement.localhost/api/orders/my?page=1&page_size=20` | `401` |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |

### PC Buyer Request Detail Real Candidates Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| PC request-detail demo removal scan | `rg -n "demoIntents\|demoSupplierCandidatesForIntent\|isDemoToken\|sortDemoCandidates\|Supplier bound for demo\|demoEnabled\|demo-" Ainerwise/modules/procurement/pc/pages/buyer/requests/[id]/index.vue` | No matches |
| Buyer candidate list | `GET http://procurement.localhost/api/intents/27569e3d-fa60-4ef6-8737-a4ea008acb65/supplier-candidates?sort=comprehensive&limit=10 -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, returned Core rows and includes catalog item `57324d5d-f13e-4545-9229-66fa96bdf2ea` |
| Buyer candidate detail | `GET http://procurement.localhost/api/intents/27569e3d-fa60-4ef6-8737-a4ea008acb65/supplier-candidates/57324d5d-f13e-4545-9229-66fa96bdf2ea?sort=comprehensive -H "Authorization: Bearer $buyer_token"` | `HTTP 200` |
| Buyer binds candidate | `POST http://procurement.localhost/api/intents/27569e3d-fa60-4ef6-8737-a4ea008acb65/supplier-candidates/57324d5d-f13e-4545-9229-66fa96bdf2ea/bind -H "Authorization: Bearer $buyer_token" -d '{"note":"PZL40 PC request detail bind smoke"}'` | `HTTP 200` |
| Cross-buyer candidate denial | Same candidate list with unrelated buyer token | `403` |
| Unauthenticated candidate denial | Same candidate list without token | `401` |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |

### PC Buyer Offer Detail Real Page Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| Modified files | `git diff --name-only -- 'Ainerwise/modules/procurement/pc/pages/buyer/offers/[id].vue' Ainerwise/modules/procurement/pc/composables/useApi.ts Ainerwise/docs/TASKS.md AINERWISE_PROCUREMENT_ZERO_LOSS_EXECUTION.md` | PC buyer offer detail page, shared PC Procurement API SDK, and task ledgers only |
| Page address | `http://procurement.localhost/buyer/offers/a9a7d493-4976-48c1-b7cb-1cf6a8bdc848` | Standalone PC Procurement offer detail route; protected by buyer middleware |
| PC buyer offer-detail placeholder removal scan | `rg -n "500 bags Portland Cement\|Cebu Builders Hub\|Republic Cement\|\\$2,150\|i\\.pravatar\|alert\\(\|router\\.push\\('/buyer/orders/1'" 'Ainerwise/modules/procurement/pc/pages/buyer/offers/[id].vue'` | No matches |
| Buyer offer detail source | Register buyer/supplier/other buyer, create supplier catalog item, create buyer intent, publish it, submit supplier offer `a9a7d493-4976-48c1-b7cb-1cf6a8bdc848`, then `GET http://procurement.localhost/api/offers/a9a7d493-4976-48c1-b7cb-1cf6a8bdc848 -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, returned id matches real Core offer |
| Cross-buyer offer detail denial | Same offer detail with unrelated buyer token | `403` |
| Unauthenticated offer detail denial | Same offer detail without token | `401` |
| Buyer awards offer | `POST http://procurement.localhost/api/offers/a9a7d493-4976-48c1-b7cb-1cf6a8bdc848/award -H "Authorization: Bearer $buyer_token"` | `HTTP 200`, created order `fa56b95b-3386-4ef1-a85b-5a24623c7f41` |
| Buyer reads awarded order | `GET http://procurement.localhost/api/orders/fa56b95b-3386-4ef1-a85b-5a24623c7f41 -H "Authorization: Bearer $buyer_token"` | `HTTP 200` |
| Procurement PC build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-pc npm run build` | PASS, Nuxt production build completed; known external asset warnings only |
| Procurement H5 build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-h5 npm run build` | PASS, Nuxt production build completed |
| Procurement Admin build | `docker compose -f docker-compose.yml -f docker-compose.procurement-standalone.yml exec -T procurement-admin npm run build` | PASS, Vite production build completed |
...
### PC Supplier Dashboard Real Aggregate Evidence

Recorded by implementation agent on 2026-06-20. Status is `READY_FOR_VERIFY`; an independent verification agent must rerun these before marking `VERIFIED`.

| Check | Command | Result |
| --- | --- | --- |
| Static dashboard sample removal scan | `rg -n "500 bags Portland Cement\|1000m Copper Wire\|Commercial AC Unit\|\\$12,450\|4\\.9\|0\\.5%\|< 15 mins\|const pings = \\[" Ainerwise/modules/procurement/pc/pages/supplier/dashboard.vue` | No matches |
| Supplier dashboard source APIs | Register supplier with `company_name`, create `/supplier/catalog/items`, register buyer, create and publish `/intents`, submit `/intents/{intent_id}/offers`, direct create `/orders`, then `GET /supplier/intents/matching`, `GET /supplier/offers`, and `GET /orders/my` with supplier token | All source APIs returned `HTTP 200`; created offer was present in `/supplier/offers`; created order was present in `/orders/my`; matching pings returned a list |
| Other supplier order isolation | `GET http://procurement.localhost/api/orders/my -H "Authorization: Bearer $other_supplier_token"` | `HTTP 200`; created order id was absent |
| Buyer denied supplier matching | `GET http://procurement.localhost/api/supplier/intents/matching -H "Authorization: Bearer $buyer_token"` | `403` |
