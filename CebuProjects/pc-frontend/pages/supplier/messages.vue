<template>
  <div class="space-y-6 h-full flex flex-col">
    <div class="flex items-center justify-between mb-2">
      <h1 class="text-2xl font-bold text-slate-900">Messages & Negotiations</h1>
    </div>

    <div class="rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 via-white to-cyan-50 px-5 py-4 shadow-sm">
      <div class="flex items-start gap-3">
        <div class="mt-0.5 rounded-full bg-emerald-100 p-2 text-emerald-700">
          <UIcon name="i-heroicons-shield-check" class="h-5 w-5" />
        </div>
        <div>
          <p class="text-sm font-semibold text-slate-900">Keep negotiation evidence inside the platform</p>
          <p class="mt-1 text-sm text-slate-600">
            External channels should only be reminders. Pricing, delivery promises, and dispute-sensitive details need to remain in the in-site thread for protection and auditability.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex bg-white border border-slate-200 rounded-lg overflow-hidden min-h-[600px] h-[70vh]">
      ...
      <div class="flex-1 flex flex-col relative bg-white">
        ...
        <div class="p-4 border-b border-slate-200 flex justify-between items-center bg-white shadow-sm z-10">
          ...
          <UButton size="xs" color="gray" variant="ghost" :to="`/supplier/orders/${activeContact.orderId}`">
...
const contacts = ref([
  {
    ...
    orderId: 'ORD-82910',
    messages: [
      { text: 'Hi, is it possible to change the delivery date to Monday?', sender: 'them', time: '10:30 AM' },
      { text: 'Can we deliver this on Tuesday instead? Monday is a public holiday in our region.', sender: 'me', time: '10:42 AM' }
    ]
  },
  {
    id: 'buy2',
    ...
    orderId: 'ORD-81105',
    messages: [
      { text: 'Can you confirm the engine specs meet the ISO standards?', sender: 'them', time: 'Yesterday 2:00 PM' },
      { text: 'Yes, I have attached the ISO certification in the order documents.', sender: 'me', time: 'Yesterday 4:15 PM' },
      { text: 'Thanks for the quick response.', sender: 'them', time: 'Yesterday 4:20 PM' }
    ]
  }
])
...
const sendMessage = async () => {
  ...
  setTimeout(async () => {
    activeContact.value.messages.push({
      text: 'Thanks! I will review this and confirm the changes on my end.',
      sender: 'them',
